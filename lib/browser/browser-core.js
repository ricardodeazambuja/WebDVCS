...
class BrowserMiniRepo {
  ...
  forceMerge(branchName, strategy = 'accept_all_target') {
    ...
    // Delegate to the core repository forceMerge implementation
    const result = this._coreRepo.forceMerge(branchName, strategy);

    // Log the merge result
    if (result.type === 'fast-forward') {
      this.log(`Fast-forward force merge of branch: ${branchName}`);
    } else if (result.type === 'three-way') {
      this.log(`Force merge of branch: ${branchName} using ${strategy} strategy (commit: ${result.commitHash?.substring(0, 8)})`);
    } else if (result.type === 'up-to-date') {
      this.log(`Already up-to-date with branch: ${branchName}`);
    }
    ...
    return {
      ...
      commitHash: result.commitHash,
      strategy: strategy,
      message: this._getForceMergeMessage(result, branchName, strategy)
    };
  }

  _getForceMergeMessage(result, branchName, strategy) {
    switch (result.type) {
      case 'fast-forward':
        return `Fast-forward force merge of '${branchName}' completed successfully`;
      case 'three-way':
        return `Force merge of '${branchName}' completed using ${strategy} strategy`;
      case 'up-to-date':
