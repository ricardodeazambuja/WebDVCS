...
class BrowserMiniRepo {
  ...
  getStorageAnalytics() {
    ...
    try {
      ...
      const deltaStats = db.exec(`
        SELECT
          COUNT(*) as delta_count,
          AVG(compression_ratio) as avg_compression,
          SUM(original_size) as total_original_size,
          SUM(delta_size) as total_compressed_size
        FROM deltas
      `);

      // Get total database size
      const totalSize = db.exec(`
        SELECT SUM(size) as total_size FROM objects
      `);

      // Process results
      const objectBreakdown = { commits: 0, trees: 0, blobs: 0 };
      let totalObjects = 0;
      let totalDbSize = 0;

      if (objectStats[0] && objectStats[0].values) {
        for (const [type, count, size] of objectStats[0].values) {
          totalObjects += count;
          totalDbSize += size || 0;

          if (type === 'commit') objectBreakdown.commits = count;
          else if (type === 'tree') objectBreakdown.trees = count;
          else if (type === 'blob') objectBreakdown.blobs = count;
        }
      }
      ...
      if (deltaStats[0] && deltaStats[0].values && deltaStats[0].values.length > 0) {
        const [deltaCount, avgCompression, totalOriginal, totalCompressed] = deltaStats[0].values[0];
        compressionRatio = avgCompression || 0;

        if (totalOriginal > 0) {
          deduplicationSavings = ((totalOriginal - totalCompressed) / totalOriginal) * 100;
        }
      }
