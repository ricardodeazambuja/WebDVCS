...
function getCommit(hash, store) {
  ...
  const commit = {
    ...
    tree: null,
    parent: null,
    parents: [],  // Array for multiple parents (merge commits)
    author: 'Unknown',
    email: null,
    timestamp: 0,
    message: ''
  ...
  for (const line of lines) {
    if (line.startsWith('tree ')) {
      ...
      const parentHash = line.substring(7);
      commit.parents.push(parentHash);
      // Keep backward compatibility - first parent in 'parent' field
      if (!commit.parent) {
...
function getCommitHistory(startHash, maxCount, store) {
  ...
  while (queue.length > 0 && commits.length < maxCount) {
    ...
    // Add parent(s) to queue for traversal
    // Handle both single parent and multiple parents (merge commits)
    ...
    } else if (commit.parent && !visited.has(commit.parent)) {
      // Single parent (regular commit)
