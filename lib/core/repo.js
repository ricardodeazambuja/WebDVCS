...
class ContentAddressedRepo {
  ...
  merge(branchName, options = {}) {
    ...
    if (mergeBase === null) {
      ...
      try {
        const mergeCommitHash = this._performThreeWayMerge(currentHead, targetHead, null, branchName, isPreview);
    ...
    try {
      const mergeCommitHash = this._performThreeWayMerge(currentHead, targetHead, mergeBase, branchName, isPreview);
      return { type: 'three-way', commitHash: mergeCommitHash, conflicts: [] };
    } catch (error) {
   ...
   * @private
   */
  _performThreeWayMerge(currentHead, targetHead, mergeBase, branchName, isPreview = false) {
    if (this.debugMode) {
    ...
    // In preview mode, don't create actual commit or update references
    if (isPreview) {
      return 'preview-merge-success'; // Placeholder to indicate merge would succeed
    }

    // Create merge commit with both parents
    const mergeMessage = `Merge branch '${branchName}' into ${this.getCurrentBranch()}`;
    const mergeCommitHash = createCommit(
      mergedTreeHash,
      mergeMessage,
      this.store.getMeta('author.name') || 'Unknown',
      this.store.getMeta('author.email') || 'unknown@example.com',
      [currentHead, targetHead],  // Both parents for proper merge commit
      this.store
    );
  ...
  /**
   * Force merge with conflict resolution strategy
   * @param {string} branchName - Name of branch to merge
   * @param {string} strategy - Resolution strategy ('accept_all_target' or 'keep_all_current')
   */
  forceMerge(branchName, strategy = 'accept_all_target') {
    const { findMergeBase, getCommit, getTree, storeTree, createCommit } = require('./objects');
    ...
    // Force merge ALWAYS creates a merge commit (never fast-forward)
    // This ensures we preserve history and clearly show conflict resolution
    return this._performForceThreeWayMerge(currentHead, targetHead, mergeBase, strategy, branchName);
  }

  /**
   * Perform forced three-way merge with conflict resolution strategy
   */
  _performForceThreeWayMerge(currentHead, targetHead, mergeBase, strategy, branchName) {
    const { getCommit, getTree, storeTree, createCommit } = require('./objects');
    ...
    for (const fileName of allFileNames) {
      ...
      // Apply three-way merge logic with forced resolution
      if (!baseEntry && currentEntry && targetEntry) {
        // File added in both - use strategy to resolve
        if (currentEntry.hash !== targetEntry.hash) {
          resolvedConflicts++;
          mergedEntries.push(strategy === 'accept_all_target' ? targetEntry : currentEntry);
        } else {
          mergedEntries.push(currentEntry); // Same content
        }
      } else if (baseEntry && !currentEntry && targetEntry) {
        // File deleted in current, modified in target
        if (baseEntry.hash !== targetEntry.hash) {
          resolvedConflicts++;
          if (strategy === 'accept_all_target') {
            mergedEntries.push(targetEntry); // Keep target's modification
          }
          // If keep_all_current, file stays deleted (don't add to mergedEntries)
        }
      } else if (baseEntry && currentEntry && !targetEntry) {
        // File modified in current, deleted in target
        if (baseEntry.hash !== currentEntry.hash) {
          resolvedConflicts++;
          if (strategy === 'keep_all_current') {
            mergedEntries.push(currentEntry); // Keep current's modification
          }
          // If accept_all_target, file stays deleted (don't add to mergedEntries)
        }
      } else if (baseEntry && currentEntry && targetEntry) {
        // File exists in all three
        if (currentEntry.hash === targetEntry.hash) {
          ...
          // File modified in both branches - use strategy to resolve
          resolvedConflicts++;
          mergedEntries.push(strategy === 'accept_all_target' ? targetEntry : currentEntry);
        }
      } else if (!baseEntry && currentEntry && !targetEntry) {
        // File added only in current
        mergedEntries.push(currentEntry);
      } else if (!baseEntry && !currentEntry && targetEntry) {
        // File added only in target
        mergedEntries.push(targetEntry);
      }
    }
    ...
    const currentBranch = this.getCurrentBranch();
    const mergeMessage = `Merge branch '${branchName}' into ${currentBranch} (resolved ${resolvedConflicts} conflicts using ${strategy})`;
    const mergeCommitHash = createCommit(
      mergedTreeHash,
      mergeMessage,
      this.store.getMeta('author.name') || 'Unknown',
      this.store.getMeta('author.email') || 'unknown@example.com',
      [currentHead, targetHead], // Both parents
      this.store
    );
